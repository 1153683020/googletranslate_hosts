...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="globalping",
        help="Use Globalping China probes first, then only probe from the current runner.",
    )
...
def local_probe(
    ...
    """Perform local TCP (80,443) and TLS (443) probing, return ranked reachable IPs."""
    # Step 1: TCP probe on ports 80 and 443
    ...
    # Step 2: TLS check on 443 (bonus filtering)
    tls_ok_ips: list[str] = []
    if picked:
        with concurrent.futures.ThreadPoolExecutor(max_workers=min(workers, len(picked))) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                if result.ok:
                    tls_ok_ips.append(result.ip)

    final_picked = tls_ok_ips or picked
    return final_picked, details


def globalping_filter(
    ip_list: list[str],
    location: str,
    limit: int,
    packets: int,
    ...
    """
    Use Globalping TCP ping (ports 80 and 443) to test given IPs.
    Returns best IPs based on number of successful ports (2 > 1 > 0) and min latency.
    """
    if not ip_list:
        return [], []

    all_details: list[dict[str, object]] = []
    scores: dict[str, tuple[int, float]] = {}  # (successful_ports_count, min_latency)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=min(workers, len(tasks))) as executor:
        future_to_task = {
            executor.submit(probe_globalping_tcp, ip, port, location, limit, packets, timeout): (ip, port)
            for ip, port in tasks
        }
        for future in concurrent.futures.as_completed(future_to_task):
            ip, port = future_to_task[future]
            results = future.result()  # list of dicts, one per probe
            for result in results:
                all_details.append(result)
                ok = result.get("ok", False)
                latency = float(result.get("latency_ms", 999999.0))
                if ok:
                    cur_count, cur_min_lat = scores.get(ip, (0, 999999.0))
                    new_count = cur_count + 1
                    new_min_lat = min(cur_min_lat, latency)
                    scores[ip] = (new_count, new_min_lat)
                else:
                    # ensure IP appears in scores even if no successful ports yet
                    scores.setdefault(ip, (0, 999999.0))

    # Rank: higher successful port count first, then lower min latency, then IP
    ranked = sorted(scores.items(), key=lambda item: (-item[1][0], item[1][1], item[0]))
    picked = [ip for ip, (cnt, _) in ranked if cnt > 0][:max_ips]
    return picked, all_details
...
def pick_ip_for_domain(
    ...
    # Step 1: Local probing (TCP + TLS)
    local_picked, local_details = local_probe(domain, candidates, timeout, workers, max_ips)
    details = local_details[:]  # copy

    # Step 2: If globalping requested and we have local candidates, test them remotely
    if probe_provider == "globalping" and local_picked:
        try:
            global_picked, global_details = globalping_filter(
                ip_list=local_picked,
            ...
            details.extend(global_details)
            if global_picked:
                return global_picked, details
            # If globalping returned no usable IP but fallback is enabled, fall through
        except Exception as exc:
            print(f"{domain}: Globalping filter failed: {exc}, fallback to local result", file=sys.stderr)

    # Step 3: Return local result if available
    if local_picked:
        return local_picked, details

    # Step 4: Fallback to DNS candidates (original behavior)
    if fallback_to_dns:
        fallback_ips = sorted(candidates)[:max_ips]
        return fallback_ips, details
